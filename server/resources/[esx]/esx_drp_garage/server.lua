...
ESX.RegisterServerCallback('eden_garage:getVehicles', function(source, cb)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles_garage WHERE owner=@identifier",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
	...
	print(plate)
		...
		for _,v in pairs(vehicules) do
			if(plate == v.plate)then
				...
				MySQL.Sync.execute("UPDATE owned_vehicles_garage SET vehicle =@vehprop WHERE id=@id",{['@vehprop'] = vehprop, ['@id'] = v.id})
				isFound = true
				break
			end		
		end
...
AddEventHandler('eden_garage:modifystate', function(vehicleProps, state)
	...
	for _,v in pairs(vehicules) do
		if(plate == v.plate)then
			local idveh = v.id
			MySQL.Sync.execute("UPDATE owned_vehicles_garage SET state =@state WHERE id=@id",{['@state'] = state , ['@id'] = v.id})
			break
		end		
	end
end)	
...
ESX.RegisterServerCallback('eden_garage:getOutVehicles',function(source, cb)	
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles_garage WHERE owner=@identifier AND state=false",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
...
AddEventHandler('eden_garage:pay', function()
	...
	TriggerClientEvent('esx:showNotification', source, 'Vous avez payer :' .. Config.Price)
...
function getPlayerVehicles(identifier)
	...
	local vehicles = {}
	local data = MySQL.Sync.fetchAll("SELECT * FROM owned_vehicles_garage WHERE owner=@identifier",{['@identifier'] = identifier})	
	for _,v in pairs(data) do
		local vehicle = json.decode(v.vehicle)
...
AddEventHandler('onMySQLReady', function()
	...
	MySQL.Sync.execute("UPDATE owned_vehicles_garage SET state=true WHERE state=false", {})
...
AddEventHandler('eden_garage:payhealth', function(price)
	...
	TriggerClientEvent('esx:showNotification', source, 'Vous avez payer :' .. price)
