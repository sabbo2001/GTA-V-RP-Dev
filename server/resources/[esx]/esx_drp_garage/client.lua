...
function OpenMenuGarage(PointType)
	...
	if PointType == 'spawn' then
		table.insert(elements,{label = "List des vehicules", value = 'list_vehicles'})
	end

	if PointType == 'delete' then
		table.insert(elements,{label = "Ranger le vehicule", value = 'stock_vehicle'})
	end

	if PointType == 'pound' then
		table.insert(elements,{label = "recuperer votre voiture de la Fourriere ("..Config.Price.."$)", value = 'return_vehicle'})
	end
...
function ListVehiclesMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		labelvehicle = vehicleName..': A la Fourriere !'
    		end	
		...
		function(data, menu)
			if(data.current.value.state)then
				...
				SpawnVehicle(data.current.value.vehicle)
					...
					exports.pNotify:SendNotification({ text = "Garage Notification: <br /> Votre Voiture est a la Fourriere!", queue = "right", timeout = 400, layout = "centerLeft" })
			end
		end,
		function(data, menu)
			menu.close()
...
function reparation(prix,vehicle,vehicleProps)
	...
	ESX.UI.Menu.Open(
		...
		{
			title    = 'Le Vehicule est endommager',
			align    = 'top-left',
			elements = elements,
		},
		function(data, menu)
			...
			if(data.current.value == 'no') then
				ESX.ShowNotification('Aller-Voir le macano ou utilisé un repair-kit')
			end
...
function ranger(vehicle,vehicleProps)
	...
	exports.pNotify:SendNotification({ text = "Garage Notification: <br /> Votre vehicle est dans le garage!", queue = "right", timeout = 400, layout = "centerLeft" })
end
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				...
				if engineHealth < 1000 then
					...
					exports.pNotify:SendNotification({ text = "Garage Notification: <br /> Ce Vehicule ne vous appartient pas!", queue = "right", timeout = 500, layout = "centerLeft" })
			end
		end,vehicleProps)
	else		
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	...
	if zone == 'spawn' then
		CurrentAction     = 'spawn'
		CurrentActionMsg  = "Appuyer sur ~INPUT_PICKUP~ pour recuperer votre vehicule"
		CurrentActionData = {}
	end

	if zone == 'delete' then
		CurrentAction     = 'delete'
		CurrentActionMsg  = "Appuyer sur ~INPUT_PICKUP~ pour ranger votre vehicule"
		CurrentActionData = {}
	end
		...
		function(data, menu)
			...
			ESX.TriggerServerCallback('eden_garage:checkMoney', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					exports.pNotify:SendNotification({ text = "Garage Notification: <br /> vous n\'avez pas assez d'argent", queue = "right", timeout = 400, layout = "centerLeft" })
