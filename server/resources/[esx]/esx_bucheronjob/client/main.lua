...
function OpenMobileBucheActionsMenu()
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_bucheron', 'Bucherons', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_bucheronjob:hasEnteredMarker', function(zone)
	...
	if zone == 'TraitementPlank' and PlayerData.job ~= nil and PlayerData.job.name == 'buche'  then
		CurrentAction     = 'plank_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end

	if zone == 'TraitementPackage' and PlayerData.job ~= nil and PlayerData.job.name == 'buche'  then
		CurrentAction     = 'TraitementPackage'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
	...
	if zone == 'BucheronsActions' and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
		CurrentAction     = 'buche_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_bucheronjob:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementBois' or zone == 'TraitementPlank') and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.BucheronsActions.Pos.x, Config.Zones.BucheronsActions.Pos.y, Config.Zones.BucheronsActions.Pos.z)
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'buche' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'plank_traitement' then
					TriggerServerEvent('esx_bucheronjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'TraitementPackage' then
					TriggerServerEvent('esx_bucheronjob:startTransform', CurrentActionData.zone)
				end
