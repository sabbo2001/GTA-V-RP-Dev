...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = 'Wood_clothes_civil', value = 'citizen_wear'},
				{label = 'Wood_clothes_vine', value = 'Wood_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'Wood_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'wood_actions_menu'
...
function OpenWoodActionsMenu()
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name ~= 'recrue' and PlayerData.job.grade_name ~= 'novice')then -- Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss'
		table.insert(elements, {label = 'take_stock', value = 'get_stock'})
	end
	...
	if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then -- Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss'
		table.insert(elements, {label = 'boss_actions', value = 'boss_actions'})
	end

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'wood_actions',
		{
			title    = 'Wood',
			align    = 'top-left',
			elements = elements
		...
		function(data, menu)
			...
			CurrentAction     = 'wood_actions_menu'
			CurrentActionMsg  = 'press_to_open'
			CurrentActionData = {}
...
function OpenMobileLumberjackActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_wood_actions',
		{
			title    = 'Wood',
			align    = 'top-left',
			elements = {
					...
					function(data, menu)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								ESX.ShowNotification('no_players_near')
							else
								local playerPed        = GetPlayerPed(-1)
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_bucheron', 'lumberjack', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_lumberjackjob:hasEnteredMarker', function(zone)
	if zone == 'Wood' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'wood_harvest'
		CurrentActionMsg  = 'press_collect'
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'CuttedWood' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'wood_traitement'
		CurrentActionMsg  = 'press_collect'
		CurrentActionData = {zone = zone}
	end		
		
	if zone == 'Planks' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'Planks_traitement'
		CurrentActionMsg  = 'press_traitement'
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'Delivery' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = 'press_sell'
		CurrentActionData = {zone = zone}
	end

	if zone == 'Bucheronaction' and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		CurrentAction     = 'wood_actions_menu'
		CurrentActionMsg  = 'press_to_open'
		CurrentActionData = {}
	end
...
AddEventHandler('esx_lumberjackjob:hasExitedMarker', function(zone)
	...
	if (zone == 'CuttedWood' or zone == 'Planks') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_lumberjackjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
		TriggerServerEvent('esx_lumberjackjob:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'wood_harvest' then
					TriggerServerEvent('esx_lumberjackjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'Planks_traitement' then
					TriggerServerEvent('esx_lumberjackjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'wood_traitement' then
					TriggerServerEvent('esx_lumberjackjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_lumberjackjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'wood_actions_menu' then
					OpenWoodActionsMenu()
				end
