...
function RemoveOwnedVehicle (plate)
  MySQL.Async.fetchAll(
    'SELECT * FROM owned_vehicles_garage',
    {},
    function (result)
      for i=1, #result, 1 do
        ...
        if vehicleProps.plate == plate then
          MySQL.Async.execute(
            'DELETE FROM owned_vehicles_garage WHERE id = @id',
            { ['@id'] = result[i].id }
          )
        end
      end
...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps)
  ...
  MySQL.Async.execute(
    'INSERT INTO owned_vehicles_garage (vehicle, owner) VALUES (@vehicle, @owner)',
    {
...
ESX.RegisterServerCallback('esx_vehicleshop:getPersonnalVehicles', function (source, cb)
  ...
  MySQL.Async.fetchAll(
    'SELECT * FROM owned_vehicles_garage WHERE owner = @owner',
    { ['@owner'] = xPlayer.identifier },
    function (result)
      local vehicles = {}
